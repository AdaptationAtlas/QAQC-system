...
function addlay0(){
    ...
    rem_sl(0);
    rem_leg(0);
    var imageId = layerSelect0.getValue();
    var image;
    //print(vismin)
    var vismax;
        if (imageId) {
      ...
      vismax = ext.lmax;
      //print('max', vismax);
      maps[0].addLayer(clip.select(0), {min: vismin, max: vismax, palette: pal}, c_name + ' '+ imageId);
      maps[0].addLayer(adm0, {}, 'Country Boundaries', false);
      });
      maps[0].centerObject(geom);
...
// Retrieve dictionary values and pass to visParam settings
  ...
  //rem_sl();
  lname = maps[0].layers().get(0).getName();
  ...
  maps[0].addLayer(result, {min: laymin,max:laymax,  palette: pal}, lname);
...
function addlay1(){
    ...
    rem_sl(1);
    rem_leg(1);
    //print('wid rem_leg pan1', maps[1].widgets())
    var imageId = layerSelect1.getValue();
    var clip;
    if (imageId) {
      // If an image id is found, create an image.
      ...
      // Add the image to the map with the corresponding visualization options.
      //var visOption = app.VIS_OPTIONS[app.vis.select.getValue()];
      clip = image.clip(geom);
      //print('addlay1 clip', clip)
      var minMax = clip.select(0).reduceRegion({reducer: ee.Reducer.minMax(), 
                                  geometry:geom, 
                                  scale: clip.projection().nominalScale(),
                                  bestEffort: true,
      ...
      vismax = ext.lmax;
      //print('max', vismax);
      maps[1].addLayer(clip.select(0), {min: vismin, max: vismax, palette: pal}, c_name + ' '+ imageId);
      maps[1].addLayer(adm0, {}, 'Country Boundaries', false);
      maps[1].centerObject(geom);
        
      });
    }
  
    clip = clip.where(clip.lte(-1000),-1);
     
// // Calculate min/max values
    var minMax1 = clip.select(0).reduceRegion({reducer: ee.Reducer.minMax(), 
                                  geometry:geom, 
                                  scale: lay.projection().nominalScale(),
                                  bestEffort: true,
                                  maxPixels: 10e15});
...
// Retrieve dictionary values and pass to visParam settings
   ...
   // add dynamic legend
    ...
    function updateLayer(){
  //rem_sl();
  lname = maps[1].layers().get(0).getName();
  //print('lname in',lname)
  maps[1].layers().reset();
  var maskvalue = slider0.getValue();
  var minval = slider01.getValue();
  ...
  //print(singleband)
  var result = singleband.updateMask(singleband.lte(maskvalue).and(singleband.gte(minval)));
  maps[1].addLayer(result, {min: laymin,max:laymax,  palette: pal}, lname)}
...
function addlay2(){
    ...
    rem_sl(2);
    rem_leg(2);
    var imageId = layerSelect2.getValue();
    var image;
    var clip;
    if (imageId) {
      // If an image id is found, create an image.
      ...
      vismax = ext.lmax;
      //print('max', vismax);
      maps[2].addLayer(clip.select(0), {min: vismin, max: vismax, palette: pal}, c_name + ' '+ imageId);
      maps[2].addLayer(adm0, {}, 'Country Boundaries', false);
      lay2 = image;
      maps[2].centerObject(geom);
      });
      //return clip;
    }
...
// Retrieve dictionary values and pass to visParam settings
    ...
    //print('panel 2 wid',maps[2].widgets());
   // add dynamic legend
    function makeLegend (colors) {
      var lon = ee.Image.pixelLonLat().select('longitude');
      var gradient = lon.multiply((colors.max-colors.min)/100.0).add(colors.min);
    ...
    // Slider function
    function updateLayer(){
      lname = maps[2].layers().get(0).getName();
      //print('lname in',lname)
      maps[2].layers().reset();
      var maskvalue = slider0.getValue();
      var minval = slider01.getValue();
      ...
      //print(singleband)
      var result = singleband.updateMask(singleband.lte(maskvalue).and(singleband.gte(minval)));
      maps[2].addLayer(result, {min: laymin,max:laymax,  palette: pal}, lname)}
      });
}
...
function addlay3(){
    ...
    var imageId = layerSelect3.getValue();
    var clip;
    if (imageId) {
      // If an image id is found, create an image.
      var image = ee.Image(imageId);
      ...
      vismax = ext.lmax;
      //print('max', vismax);
      maps[3].addLayer(clip.select(0), {min: vismin, max: vismax, palette: pal}, c_name + ' '+ imageId);
      maps[3].addLayer(adm0, {}, 'Country Boundaries', false);
      lay3 = image;
      maps[3].centerObject(geom);
...
// Retrieve dictionary values and pass to visParam settings
    ...
    var legend;
    function rem_leg(map_pan){
        legend = maps[map_pan].widgets().get(3);
        if(legend){
                maps[map_pan].remove(legend);
                //print(maps[map_pan].widgets());
   ...
   // add dynamic legend
    ...
    function updateLayer(){
    //rem_sl();
    lname = maps[3].layers().get(0).getName();
    //print('lname in',lname)
    maps[3].layers().reset();
    var maskvalue = slider0.getValue();
    var minval = slider01.getValue();
    ...
    //print(singleband)
    var result = singleband.updateMask(singleband.lte(maskvalue).and(singleband.gte(minval)));
    maps[3].addLayer(result, {min: laymin,max:laymax,  palette: pal}, lname)}
...
function addlay4(){
    ...
    var imageId = layerSelect4.getValue();
    var clip;
    if (imageId) {
      // If an image id is found, create an image.
      var image = ee.Image(imageId);
      ...
      vismax = ext.lmax;
      //print('max', vismax);
      maps[4].addLayer(clip.select(0), {min: vismin, max: vismax, palette: pal}, c_name + ' '+ imageId);
      maps[4].addLayer(adm0, {}, 'Country Boundaries', false);
      lay4 = image;
      maps[4].centerObject(geom);
...
// Retrieve dictionary values and pass to visParam settings
   ...
   // add dynamic legend
    ...
    // Slider function
    function updateLayer(){
    lname = maps[4].layers().get(0).getName();
    //print('lname in',lname)
    maps[4].layers().reset();
    var maskvalue = slider0.getValue();
    var minval = slider01.getValue();
    ...
    //print(singleband)
    var result = singleband.updateMask(singleband.lte(maskvalue).and(singleband.gte(minval)));
    maps[4].addLayer(result, {min: laymin,max:laymax,  palette: pal}, lname)}
...
function addlay5(){
    ...
    var imageId = layerSelect5.getValue();
    var clip;
    if (imageId) {
      // If an image id is found, create an image.
      var image = ee.Image(imageId);
      ...
      vismax = ext.lmax;
      //print('max', vismax);
      maps[5].addLayer(clip.select(0), {min: vismin, max: vismax, palette: pal}, c_name + ' '+ imageId);
      maps[5].addLayer(adm0, {}, 'Country Boundaries', false);
      maps[5].centerObject(geom);
...
// Retrieve dictionary values and pass to visParam settings
   ...
   // add dynamic legend
    ...
    // Slider function
    function updateLayer(){
    lname = maps[5].layers().get(0).getName();
    //print('lname in',lname)
    maps[5].layers().reset();
    var maskvalue = slider0.getValue();
    var minval = slider01.getValue();
    ...
    //print(singleband)
    var result = singleband.updateMask(singleband.lte(maskvalue).and(singleband.gte(minval)));
    maps[5].addLayer(result, {min: laymin,max:laymax,  palette: pal}, lname)}
 
  });
  
  diff_button.setDisabled(false);
}
...
function addlay6(){
    ...
    var imageId = layerSelect6.getValue();
    var clip;
    if (imageId) {
      // If an image id is found, create an image.
      var image = ee.Image(imageId);
      ...
      vismax = ext.lmax;
      //print('max', vismax);
      maps[6].addLayer(clip.select(0), {min: vismin, max: vismax, palette: pal}, c_name + ' '+ imageId);
      maps[6].addLayer(adm0, {}, 'Country Boundaries', false);
      maps[6].centerObject(geom);
...
// Retrieve dictionary values and pass to visParam settings
   ...
   // add dynamic legend
    ...
    // Slider function
    function updateLayer(){
    lname = maps[6].layers().get(0).getName();
    //print('lname in',lname)
    maps[6].layers().reset();
    var maskvalue = slider0.getValue();
    var minval = slider01.getValue();
    ...
    //print(singleband)
    var result = singleband.updateMask(singleband.lte(maskvalue).and(singleband.gte(minval)));
    maps[6].addLayer(result, {min: laymin,max:laymax,  palette: pal}, lname)}
...
function col_select(){
     ...
     linker.forEach (function addlay (map_panel){
